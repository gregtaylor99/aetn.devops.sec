...
##curl -k -u "$USERNAME:$PASSWD" -H "content-type: text/xml" https://qualysapi.qg2.apps.qualys.com/qps/rest/3.0/search/was/webapp -X "GET" -L
...
#curl -u $USERNAME:$PASSWD -d "action=launch&template_id=143409&output_format=pdf" -H "X-Requested-With: Curl" "https://qualysapi.qg2.apps.qualys.com/api/2.0/fo/report/" 
...
case $1 in  
   ...
   aetv.2)
      curl -u $USERNAME:$PASSWD -H "X-Requested-With: Curl" "https://qualysguard.qg2.apps.qualys.com/api/2.0/fo/appliance/?action=list&echo_request=1"
       ;;
   aetv.3)  
      curl -vvv -u aenet2nm:ae123 -H "content-type: text/xml" -X "POST" --data-binary @- "https://qualysapi.qg2.apps.qualys.com/qps/rest/3.0/count/was/webapp" < file.xml
